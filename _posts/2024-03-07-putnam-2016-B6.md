...
So the truth of the $m\text{th}$ case implies the truth of the $(m+1)\text{th}$ case, and the base case implies it's true for $m\geq0$.

Now, let $|x|<0$ and let $m\to\infty$ and we get $(1)$. Taking the logarithm of this infinite product yields
...
We'd like to switch the order of summation, so we must verify that the double sum converges absolutely. Here, we'll take $x\in(0,\,1),$
