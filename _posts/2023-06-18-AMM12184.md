...
where $R>\text{max}\left\{1,\,1/\sqrt{\alpha}\right\}$, $0<\epsilon<\text{min}\left\{1,\,1/\sqrt{\alpha}\right\}$, and $C_{R,\,\epsilon}$ is the following keyhole contour (here the branch cut for $z^{1/2}$ is on the real axis) in the following diagram.

![](https://github.com/VolumeElement/VolumeElement.github.io/blob/main/images/AMM12184_edit.png)
